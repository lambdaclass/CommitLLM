...
- Output-policy enforcement now covers `eos_policy`, `min_tokens`, `ignore_eos`, stop conditions, detokenization policy, and fail-closed handling for unsupported or unknown policy values.
...
- Decode-spec handling is fail-closed for unsupported sampler versions; only the canonical sampler version is accepted.
