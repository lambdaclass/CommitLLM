/**
 * Deterministic Attention Kernel — bit-exact against CPU reference.
 *
 * Single-query decode attention with fully specified arithmetic:
 *   Step 1: Score = Q · K^T * inv_sqrt_d  (sequential f32, no FMA)
 *   Step 2: Softmax with canonical exp polynomial
 *   Step 3: Output = P @ V  (sequential f32, no FMA)
 *
...
/**
 * Deterministic decode attention kernel.
 *
 * One block per query head. Thread 0 computes scores + softmax (sequential).
 * All threads compute V aggregation (parallel across d_head).
 *
 * inv_sqrt_d is precomputed by the caller — same f32 bits on CPU and GPU.
 */
__global__ void deterministic_attention_kernel(
    ...
    int d_head,
    int seq_len,
    float inv_sqrt_d   // precomputed by caller, same bits as CPU
) {
    int qh = blockIdx.x;
    ...
    // --- Step 1: Scores (thread 0, sequential) ---
    if (threadIdx.x == 0) {
