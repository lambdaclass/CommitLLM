...
| 2 | **Real-model corridor attacks** — run corridor amplification on Qwen-7B and Llama-8B with production weights, prioritized on the bad layers identified by #1 rather than uniformly across all layers. Determine whether ±τ perturbations can flip tokens on trained models (not just random-weight toy), and whether only early/final layers dominate the risk. Subtests: constrained worst-case search under `‖Δa‖∞ ≤ τ` and `‖Δa‖2` caps, bad-layer localization, multi-token accumulation across autoregressive decode, and multi-prompt diversity (code, math, long-context, low-margin, high-margin prompts) so the results are not tied to one prompt with an unusually small or large logit margin. | open |
