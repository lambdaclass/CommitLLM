...
3. **Finish the captured-logits economics split** — baseline retained/open/full-tier numbers are now measured; the remaining gating question is the isolated delta from capture itself. Run the A/B benchmark (`capture off` vs `capture on`) to separate: (a) hook/copy overhead ms/token, (b) commit/finalize delta, (c) routine-tier payload + verify, (d) full-tier payload + verify, and (e) peak host memory under cached key/artifact conditions. Treat keygen as an offline artifact build, not a steady-state cost.
4. **Fix arbitrary-position dense attention ASAP** — do not widen tolerances to make tests pass. Run one strict FlashAttention/kernel-aligned witness attempt that replays the **same online-softmax recurrence the kernel uses**, including the running max/normalizer/output accumulator over KV tiles, not a global CPU softmax. If that cannot produce honest low-bandwidth verification, promote deterministic attention kernels. **(Main correctness blocker.)**
...
2. **Attention takes one honest branch** — either the Tier 1/2/3 FlashAttention-aligned witness passes broad randomized sweeps with the stated payload/time targets, or the stock-kernel path is explicitly rejected and deterministic attention kernels are promoted.
3. **Docs/guarantees match the kept path** — README, roadmap, benchmarks, and publication language state one dense-model story without replay/tolerance ambiguity.
...
4a. **Tier 1 witness spike (no kernel modification)** — reuse existing witnessed-score infrastructure but replace global CPU softmax with the kernel-shaped **online-softmax replay**: tile over KV with the actual FA2 block size, maintain running `m`, `l`, and `O` (output accumulator), and avoid separate global weights materialization. Use the same scaled-exp structure the kernel uses (`exp2`-shaped scaling where applicable), not only a tiled normalization pass. Goal: determine whether the current `L∞ ≈ 8–9` gap collapses once the replay matches the kernel recurrence rather than textbook global softmax.
4b. **Tier 2 witness spike (+softmax LSE capture)** — if Tier 1 is close but not clean, add minimal hook-level capture of softmax LSE / equivalent per-head summary from the FA call to bind the CPU replay to the actual GPU normalization state. Payload target: negligible relative to shell/decode (few KB total per challenged token, not MB-scale).
4c. **Tier 3 witness spike (+per-tile state) only if necessary** — if Tier 1/2 still miss, emit per-tile online-softmax state (`m`, `l`, and/or `O`) from a modified FA2 path. This is the last stock-kernel attempt before deterministic kernels. Keep payload target in the single-digit-to-low-double-digit MB range per fully attention-audited token.
...
| 8a | **Dense FlashAttention-aligned attention witness — ASAP** — replace rejected f64 softmax·V replay with a kernel-aligned replay/witness experiment for Llama and Qwen. The immediate target is **decode-time attention only** (`seqlen_q = 1`) on the actual vLLM/FlashAttention-2 path, not a generic attention abstraction. Tiered plan: **Tier 1** — no kernel modification, reuse existing witnessed-score captures but replay the **full online-softmax recurrence** over the actual KV tile shape (running `m`, `l`, and output accumulator `O`), avoiding separate global weights materialization; **Tier 2** — if Tier 1 is close but not clean, add minimal hook-level capture of softmax LSE / equivalent per-head normalization summaries; **Tier 3** — only if needed, emit per-tile online-softmax state (`m`, `l`, `O`) from a modified FA2 path; **Tier 4** — deterministic attention kernels. Captured `a` alone is not sufficient; it is only the committed downstream boundary. Hard promotion criteria: honest pass on broad randomized prompt sweeps on both Llama and Qwen, no loose empirical tolerance story, payload stays in the single-digit-to-low-double-digit MB range per fully attention-audited token, and verifier attention time stays in the ms / low-tens-of-ms range. If Tier 1 does not sharply collapse the current `L∞ ≈ 8–9` gap, escalate quickly rather than iterating on replay tolerances. | **ASAP / open** |
...
| 99 | Update docs with the kept dense-model story: sampled decode exact via `CapturedLogits`; arbitrary-position attention still open until the witness/kernel path lands; measured decode economics separated into retained/open/routine/full costs; no stale replay-tolerance language |
