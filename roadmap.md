...
| # | Item | Status |
|---|------|--------|
| 1 | **Local attention operator on real models** — measure the actual local map from `Δa_i8` to the next verified state on the exact served Qwen/Llama checkpoints, not just the crude scalar `ρ_j = γ·‖W_o‖₂/RMS(x^(j))`. Record both spectral (`2→2`) and `L∞`-compatible bounds for `T_j = bridge ∘ J_RMSNorm(x_j) ∘ W_o^(j) ∘ scale_a_j`, and identify the genuinely dangerous layers. Use the RMSNorm contraction ratio as a cheap first diagnostic, but do not treat it as the final theorem by itself. | open |
...
| 7 | **Score witnessing** — commit pre-softmax scores S=Q·K^T/√d per audited token. Verifier checks S against canonical QK^T from shell-verified Q and committed K, then recomputes softmax(S)·V independently. Treat this as a selective deep-audit fallback for the bad layers or high-risk contexts identified by #1 and #2, not as the default routine path. Not needed if deterministic replay (#67) achieves τ=0 with a defined arithmetic spec. Cost: O(seq_len × n_heads) per audited token per layer, deep audit only. | open |
| 8 | **W_o conditioning** — compute σ_min(W_o), ‖W_o‖₂, and more faithful projected/local operator bounds per layer on real models. Quantifies adversary freedom when attn_out_i8 is commitment-bound but not independently replayed, and helps decide whether only a small set of layers need deeper audit. | open |
