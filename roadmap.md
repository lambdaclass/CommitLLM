...
1. [ ] Rewrite the verifier from scratch against the frozen final spec, done when a new minimal linear verifier consumes the canonical receipt/audit format, passes the golden vectors, and replaces transitional branches in the trusted verification path. The rewrite is not done until its testing story is also explicit: phase-level tests for `Ctx::new()` and each verifier phase, legacy-vs-canonical parity tests on overlapping cases, canonical frozen pass/reject fixtures, full verifier-suite coverage (`canonical`, `v4_e2e`, `boundary_fuzz`, `cross_version`, `golden_conformance`, `hardening_gate`), and real-GPU confirmation through `scripts/modal/test_e2e_v4.py` and `scripts/modal/test_adversarial.py`.
