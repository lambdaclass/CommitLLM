...
The shell/tail/binding protocol is structurally complete. The Freivalds shell checks are gated by verification profile: Llama checks all 7 matrices (`Wq/Wk/Wv/Wo/Wg/Wu/Wd`); Qwen skips `Wq/Wk/Wv` because the bridge replay cannot match the GPU's quantized QKV GEMM (exact QKV Freivalds remains Llama-only until attention-side local verification or deterministic kernels close the gap). Token-identity acceptance is also profile-gated: Llama uses `ExactTokenIdentity`; **Qwen decode is now shipped** via `DecodeAcceptanceMode::LpHiddenBf16` on the binary-key path. The decode boundary is the hidden state at `LogitsProcessor args[1]`: captured LP hidden + verifier-side `bf16 lm_head` replay passes 261/261 verifier checks on real Qwen W8A8 weights for both greedy and sampled decode, at ~7 KB/token capture cost. The `lm_head` Freivalds matmul check remains exact for all profiles. The remaining open problems are: (1) attention-side / prefix-side local verification, (2) verifier key size (1.57 GB for Qwen 7B due to `lm_head_bf16` — binary key path mandatory, key distribution is a product concern). Tiers differ in attention evidence and payload cost.
...
4. ~~**Prototype the decode-side captured boundary**~~ — **SHIPPED.** LP hidden → bf16 lm_head → canonical sampler passes 261/261 checks on real Qwen W8A8 weights (greedy and sampled). Binary key path mandatory (1.57 GB key). Decode correctness is solved.
5. ~~**Promote the LP-hidden decode path end-to-end**~~ — **DONE.** Real-weight Qwen E2E passed on Modal A100-80GB. Docs updated. Remaining follow-up: capture overhead benchmark, key size optimization (`#4c`, `#20`, `#25c`).
...
| 4b | ~~**Decode boundary at `LogitsProcessor` input — SHIPPED**~~ — LP-hidden capture passes exact hook-level validation (23/23 checks) and real-weight GPU E2E (261/261 verifier checks, 3 greedy + 2 sampled prompts on Modal A100-80GB). Protocol surface: committed `lp_hidden_bf16`, `DecodeAcceptanceMode::LpHiddenBf16`, verifier-side `bf16 lm_head` replay, keygen `lm_head_bf16` population, binary key API. ~7 KB/token capture cost. | **done** |
| 4c | ~~**Qwen real-weight LP-hidden E2E — PASSED**~~ — real prover/verifier path on Modal A100-80GB with keygen-populated `lm_head_bf16`: 261/261 checks on all runs (greedy × 3, sampled × 2, EOS trim). Binary key path mandatory (1.57 GB for Qwen 7B). Remaining follow-up: capture overhead benchmark (sync D2H), key size optimization (lm_head_bf16 is 544M u16 = 1.57 GB — consider separate artifact or compression). | **done** |
