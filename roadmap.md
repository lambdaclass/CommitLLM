...
| # | Item | Status |
|---|------|--------|
| 1 | **RMSNorm contraction — MEASURED** — ρ_j = γ·‖W_o‖₂/RMS(x^(j)) measured on real checkpoints. **Qwen-7B W8A8**: 24/28 contract (86%), 4 early layers expand (ρ up to 63.8), drift/residual=0.63. **Llama-3.1-8B**: 0/32 contract (0%), every layer expands (ρ 1.7–691), drift bound diverges. RMSNorm contraction alone does NOT close the corridor for Llama. Score witnessing (#7) and W_o conditioning (#8) are critical. Next step: measure the full local operator `T_j = bridge ∘ J_RMSNorm(x_j) ∘ W_o^(j) ∘ scale_a_j` to get tighter per-layer bounds and identify which layers are genuinely dangerous vs just having conservative ρ estimates. | **measured** |
| 2 | **Real-model corridor attacks** — run corridor amplification on Qwen-7B and Llama-8B with production weights, prioritized on the bad layers identified by #1 rather than uniformly across all layers. Determine whether ±τ perturbations can flip tokens on trained models (not just random-weight toy), and whether only early/final layers dominate the risk. | open |
| 3 | **Unify attention path** — one `x_attn` boundary, one QKV replay story across prover/verifier/corridor. No silent mixing of bridge-derived Q with committed K/V. | open |
| 4 | **Adversarial testing** — cheating-provider test suite: receipt forgery, transcript splicing, intermediate tampering, model substitution, selective layer cheating, KV injection, and selective-abort / audit-policy gaming. Every attack must fail verification or be documented as accepted gap. Includes real-model constrained corridor attacks (see #2). | **in progress** |
| 5 | **Formal security argument** — define the cheating game, state Freivalds soundness bound (1/p), state detection probability as f(sampling rate, cheating fraction), state what commitment binding prevents. | open |
...
| 7 | **Score witnessing** — commit pre-softmax scores S=Q·K^T/√d per audited token. Verifier checks S against canonical QK^T from shell-verified Q and committed K, then recomputes softmax(S)·V independently. **Confirmed critical by #1**: Llama has ρ>1 at every layer, so corridor tolerance is not self-dampening. Selective deep-audit fallback for bad layers or high-risk contexts. Not needed if deterministic replay (#67) achieves τ=0 with a defined arithmetic spec. Cost: O(seq_len × n_heads) per audited token per layer, deep audit only. | open |
| 8 | **W_o conditioning** — compute σ_min(W_o), ‖W_o‖₂, and more faithful projected/local operator bounds per layer on real models. **Confirmed critical by #1**: Llama ρ>1 everywhere means the crude spectral bound is loose — need tighter per-layer operator norms to identify which layers genuinely amplify vs where the bound is just conservative. Quantifies adversary freedom when attn_out_i8 is commitment-bound but not independently replayed. | open |
| 9 | **Deep-audit payload sizing** — budget the per-token cost of score witnessing and KV provenance at production context lengths (4K, 32K, 128K). Determine storage, bandwidth, and verification-time scaling, including whether score witnesses are marginal compared with the already-opened full-prefix KV payload and whether targeted bad-layer score witnessing keeps the cost acceptable. Informs audit tier design and whether long-context deployments need different policies. | open |
