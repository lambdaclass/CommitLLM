...
The shell/tail/binding protocol is structurally complete. The Freivalds shell checks are gated by verification profile: Llama checks all 7 matrices (`Wq/Wk/Wv/Wo/Wg/Wu/Wd`); Qwen skips `Wq/Wk/Wv` because the bridge replay cannot match the GPU's quantized QKV GEMM (exact QKV Freivalds remains Llama-only until attention-side local verification or deterministic kernels close the gap). Token-identity acceptance is also profile-gated: Llama uses `ExactTokenIdentity`; **Qwen decode is now shipped** via `DecodeAcceptanceMode::LpHiddenBf16` on the binary/decode-artifact path. The decode boundary is the hidden state at `LogitsProcessor args[1]`: captured LP hidden + verifier-side `bf16 lm_head` replay passes 261/261 verifier checks on real Qwen W8A8 weights for both greedy and sampled decode, at ~7 KB/token capture cost. On the attention side, the profile split is now explicit: **Llama stays on exact replay** while **Qwen uses witnessed-score attention**. The `lm_head` Freivalds matmul check remains exact for all profiles. The remaining open problems are: (1) attention-side / prefix-side local verification, (2) large decode-artifact distribution/storage (Qwen 7B `lm_head_bf16` is still ~1.57 GB even after splitting it out of `VerifierKey`), (3) defining a practical audit policy that balances decode-all coverage, partial attention coverage, and a small number of fully opened shell audits. Tiers differ in attention evidence and payload cost.
...
4. ~~**Prototype the decode-side captured boundary**~~ — **SHIPPED.** LP hidden → bf16 lm_head → canonical sampler passes 261/261 checks on real Qwen W8A8 weights (greedy and sampled). Large decode artifact mandatory; decode correctness is solved.
5. ~~**Promote the LP-hidden decode path end-to-end**~~ — **DONE.** Real-weight Qwen E2E passed on Modal A100-80GB. Docs updated. Remaining follow-up: capture overhead benchmark, decode-artifact packaging/distribution (`#4c`, `#20`, `#25c`).
...
8. **Establish one exact dense-attention reference tier** — implement the brute-force exact attention audit for one challenged Qwen token from committed KV transcript and measure the real payload / CPU cost before optimizing (`#7a`, `#10`, `#24`).
9. **Prototype the compact stock-kernel attention witness** — for Qwen, test score witnessing plus a compact `weights @ V -> a` proof/sketch before escalating to deterministic kernels or DeepSeek-V3. This is now the best remaining low-bandwidth stock-kernel path because brute-force exact KV replay is too expensive and not honest at later positions (`#8a`, `#10`, `#24`, `#25`).
10. **Take the next production-relevant frontier jump with DeepSeek-V3** — once Qwen attention is closed well enough to be honest about the dense story, start the DeepSeek-V3 track (MoE + MLA + MTP) instead of treating a third dense family as a hard prerequisite (`#29a`, `#29`, `#35b`, `#35d`, `#35c`).
11. **Broaden dense coverage and portability** — third dense family, larger dense model, longer context, and one non-W8A8 dense quant path (`#31`, `#31a`, `#32`, `#33`, `#34`, `#35`).
12. **Take the broader frontier jump** — FP8 boundary policy first (`#30a`), then FP8 KV and wider architecture variants (`#30`, `#35a`, `#35e`).
13. **Improve serving/runtime compatibility** — native capture (including native captured-boundary backend `#36a`), batching, paging, TP, CUDA graphs, payload reduction (`#36` onward).
14. **Pursue verified-mode research** — deterministic kernels / deterministic runtime only after the current stock-kernel path is shipped and benchmarked (`#67`, `#68`).
...
5. **Exact dense-attention audit ships as a reference tier before optimization** — for Qwen, land one brute-force exact attention audit for a challenged token using committed KV transcript and canonical attention replay. Use the measured payload/CPU cost as the reference upper bound before introducing score witnesses, sketches, or compact row witnesses.
6. **Compact attention witness is the next stock-kernel gate if brute-force replay is too expensive** — after the exact reference tier is benchmarked, test score witnessing plus a compact `V`-side proof/sketch before escalating to deterministic kernels. Only if the compact witness fails to give an honest low-bandwidth path should kernels move forward as the next mainline attention fix.
7. **Native captured-boundary backend is conditional, not automatic** — only invest in the native path after the protocol explicitly chooses local captured boundaries as the kept dense-model direction and after the boundary cost benchmark shows Python copy/materialization is the limiting factor. The LP-hidden decode path may justify native capture sooner if synchronous D2H proves too expensive in production.
...
8. `#7a` land one brute-force exact Qwen attention audit from committed KV transcript
9. `#8a` prototype Qwen score-witness + compact-`V` proof path
10. `#2i` benchmark boundary capture cost and separate prover-internal capture volume from client-visible audit payload
11. `#10` measure real audit bandwidth / CPU cost and define the production audit policy
12. `#2h` define strong-tier boundary policy per family from measured results, not assumptions
13. `#8` freeze generated-token score witnessing as the strong tier
14. `#15` freeze the canonical verifier
15. `#14` keep GPU smoke tests green
16. `#20` update docs/claims
17. `#21` cut the release
...
| 2h | **Strong-tier boundary policy** — for each supported family, specify which boundaries are: (a) captured exactly, (b) locally checked, (c) replayed approximately, (d) unsupported. This prevents the protocol from silently mixing bridge-derived and captured semantics across audit tiers. Current intended split: Llama exact replay, Qwen witnessed-score attention, decode exact on both families where supported. | partial |
...
| 4b | ~~**Decode boundary at `LogitsProcessor` input — SHIPPED**~~ — LP-hidden capture passes exact hook-level validation (23/23 checks) and real-weight GPU E2E (261/261 verifier checks, 3 greedy + 2 sampled prompts on Modal A100-80GB). Protocol surface: committed `lp_hidden_bf16`, `DecodeAcceptanceMode::LpHiddenBf16`, verifier-side `bf16 lm_head` replay, keygen `lm_head_bf16` population, binary/decode-artifact API. ~7 KB/token capture cost. | **done** |
...
| 8 | **Score witnessing** — generated-token score witnessing is now the kept strong-tier direction. **Llama:** bf16 score anchoring is validated, but Llama still stays on exact replay as the canonical attention mode unless measurements force otherwise. **Qwen:** with captured `x_attn` as the QKV boundary (#2f), shell K is exact and witnessed-score attention is now the intended strong-tier path because raw exact replay fails at later tokens. Current measured state: last-token Qwen audits on short / medium / long prompts reduce the replay tail to a deterministic `max_diff = 2–3` on only a few layers, but the final `±3` bound is **not frozen yet**. Before the docs/claims promote this as a shipped strong-tier rule, run a broader randomized sweep across prompt families, longer contexts, and sampled generated positions to confirm the tail stays at `≤ 3`. Keep the generated token exact by default; sample prefix tokens only as a later probabilistic extension. Compression ideas (top-k + tail bounds) are optimization work after the plain witnessed-score tier is benchmarked. | **partial** |
| 8a | **Qwen compact attention witness** — prototype the concrete low-bandwidth stock-kernel answer after `#7a`: prover witnesses the pre-softmax score rows for the challenged token, verifier replays canonical softmax, and the protocol uses a compact `V`-side linear proof/sketch for `weights @ V -> a` instead of opening the full `V` history. The explicit routing machinery is now in place (`AttentionVerificationMode::WitnessedScores`), but the current witnessed-score phase still relies on the existing committed `V` side rather than the compact proof/sketch. Current measured state: with the present `V` side, honest last-token Qwen runs are close but not yet fully closed (`2–5 / 28` failing layers, `max_diff = 2–3`, payloads roughly `13 MB` at ~43 tokens, `33 MB` at ~136, `117 MB` at ~532). Remaining success criteria: broader randomized validation of the `≤ 3` tail, fixing the separate last-token `lp_hidden_bf16` opening bug, then reducing payload to single-digit-to-low-double-digit MB per fully attention-audited token via the compact proof/sketch. If this path fails to produce an honest low-bandwidth bound, deterministic kernels become the next mainline attention fix. | **in progress** |
...
| 10 | **Deep-audit payload sizing / policy** — budget the per-token cost of score witnessing, KV provenance, and the exact-attention reference tier at production context lengths (4K, 32K, 128K). Determine storage, bandwidth, and verification-time scaling. **Must benchmark CPU verification time in the Rust verifier**, not estimate from FLOPs (likely memory-bound). Note: for Fiat-Shamir-sampled decode tokens, average prefix length applies, not max context. Document one practical default policy explicitly: decode on all generated tokens, sampled attention-row checks on all generated tokens, and a small number of full-shell / full-attention token audits. Use measured numbers, not FLOP estimates, to set those tiers. Before freezing any witnessed-score tolerance, require a broader randomized sweep across prompt families, lengths, and sampled generated positions. | open |
...
| 25c | **Decode acceptance semantics** — specify per profile whether final-token acceptance is exact token identity, bounded token acceptance under quantized replay, unsupported, or captured-boundary exact (`LpHiddenBf16`). This is separate from `lm_head` Freivalds: the linear algebra check can remain exact while the decode-acceptance rule changes by profile. Current state: Llama uses `ExactTokenIdentity`; Qwen `LpHiddenBf16` is shipped on the binary-key / decode-artifact path. Document how greedy vs sampled decode is handled, what randomness is or is not replayed exactly, and how each decode mode is reported to users. |
...
1. `#29a` DeepSeek-V3 frontier feasibility / compact-policy track
2. `#31` Third dense family (`Mistral` / `Ministral` / `Nemo` or `Gemma`)
3. `#30` FP8 support
