...
13. [ ] Update guarantee language to match actual verification boundaries, done when the protocol docs clearly separate exact (shell/tail/bindings), statistical (sampled provenance checks), approximate (FP attention replay), and fail-closed (versioning/unsupported paths) guarantees. This wording should also make explicit that approximate replay checks are non-state-replacing: they gate acceptance but do not overwrite the committed trajectory used by later exact checks.
