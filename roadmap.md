...
| # | Item | Status |
|---|------|--------|
| 1 | **RMSNorm contraction on real models** — measure ρ_j = γ·‖W_o‖₂/RMS(x^(j)) at every layer of Qwen-7B and Llama-8B. If ρ<1 uniformly, the per-layer ±τ corridor tolerance is already safe: each injection gets dampened before it reaches the residual, and accumulated drift is negligible. This is the cheapest measurement that can resolve the corridor question — one forward pass, dump residual norms and weight spectral norms. If ρ<1, score witnessing and W_o conditioning drop in priority. If ρ>1 on some layers, those items become urgent. | open |
| 2 | **Real-model corridor attacks** — run corridor amplification on Qwen-7B and Llama-8B with production weights. Determine whether ±τ perturbations can flip tokens on trained models (not just random-weight toy). If contraction measurement (#1) shows ρ<1, this confirms the theory. If ρ>1 on some layers, this quantifies the actual risk. | open |
...
| 7 | **Score witnessing** — commit pre-softmax scores S=Q·K^T/√d per audited token. Verifier checks S against canonical QK^T from shell-verified Q and committed K, then recomputes softmax(S)·V independently. Fallback for deployments using nondeterministic fast kernels (FlashAttention) where τ>0. Not needed if deterministic replay (#67) achieves τ=0 with a defined arithmetic spec. Priority depends on contraction measurement (#1): if ρ<1, this is optional hardening; if ρ>1, this is critical. Cost: O(seq_len × n_heads) per audited token per layer, deep audit only. | open |
| 8 | **W_o conditioning** — compute σ_min(W_o) and ‖W_o‖₂ per layer on real models. Quantifies adversary freedom when attn_out_i8 is commitment-bound but not independently replayed. Directly informs how tight the corridor needs to be. Priority depends on contraction measurement (#1). | open |
| 9 | **Deep-audit payload sizing** — budget the per-token cost of score witnessing and KV provenance at production context lengths (4K, 32K, 128K). Determine storage, bandwidth, and verification-time scaling, including whether score witnesses are marginal compared with the already-opened full-prefix KV payload. Informs audit tier design and whether long-context deployments need different policies. | open |
...
| 23 | **Tolerance bounds** — analytical attention-gap story for the kept path. Empirical data exists (Qwen L∞=8, Llama L∞=9); a full composed theorem does not. Do not assume plain per-layer `L∞` alone gives a strong semantic guarantee. Combine per-step corridor bounds with real-model attack evidence (#2), `W_o` conditioning (#8), and score witnessing (#7) where needed. RMSNorm contraction is supporting evidence, not the primary fix. |
