...
11. ~~**Align verifier/docs/reporting to the kept stock-mode attention claim**~~ — **DONE (2026-04-24).** `StockBounded` / `verified-attention` product language is removed from the verifier hot path and stock-mode reports. The kept stock-mode names and statuses are now explicit (`AuditedInputsOnly`, `AuditedInputsNotVerified`, `*_audited` profiles). README, roadmap, and CHANGELOG all state the same product claim: exact decode; attention inputs/wiring are audited; arbitrary-position attention outputs are not verified. Paper revision remains split out as item 11a.
11a. **Revise paper to match audit-only attention stance (deferred)** — `paper/main.typ` currently specifies attention replay (§Attention Replay, Proposition 2, Table 1 CommitLLM row) as part of the protocol. Revise to drop approximate replay as a protocol step and state the audit-only claim: exact decode, audited attention inputs/wiring (score anchoring, KV provenance, mask/RoPE/GQA, local replay smoke), no arbitrary-position attention verification. Deferred — schedule after code/verifier/reporting are aligned; do not touch `paper/main.typ` before then.
12. ~~**Replace stock-mode attention reporting with honest audit statuses**~~ — **DONE (2026-04-24).** Stock-mode attention reports now use audit-only statuses and structured audit fields instead of certification outcomes.
13. ~~**Wire exact score anchoring into the stock-mode audit path**~~ — **DONE (2026-04-25).** Challenged audited-profile runs now recompute `QK^T / sqrt(d)` from opened `Q/K` and compare against witnessed scores. Current live scope: the score witness covers the last generated token.
14. ~~**Wire KV provenance and wiring audits into the stock-mode audit path**~~ — **DONE (2026-04-25).** KV provenance plus GQA / RoPE / mask wiring audits are verifier-live and E2E-green on `llama-w8a8-audited` and `qwen-w8a8-audited`.
15. **Keep only token-0 / local exact replay as regression smoke** — token-0 smoke remains the kept exact attention regression check. Broader local replay smoke is optional hardening only and must not widen the product claim.
16. **Run the stock-mode attention anti-cheat / detectability campaign** — measure the real remaining hole under the audit-only contract: single-layer fake-`a`, sparse-layer cheating, early-vs-late layer attacks, last-token-witness evasion, and local-replay evasion. The question is not "is attention verified?" but "how hard is useful cheating and how often is it caught under the shipped audit policy?" **First pass landed (2026-04-26).** A0/A1 measured on the toy bridge (`crates/verilm-verify/tests/redteam_fake_a.rs`): post-capture mutation is reliably caught (A0); a *consistent* fake-`a` forgery clears the audit-only verifier (10/10 verifier passes, 4/10 undetected answer changes). Honest reading: *"audited but not verified; post-hoc tampering is detected; consistent fake-`a` remains a known residual hole."* Methodology and results: [`docs/security/redteam_audit_only.md`](./docs/security/redteam_audit_only.md). A2–A5 (sparse multi-layer, last-token-witness evasion, canary prompts, local replay smoke) are tracked as backlog and do not move the headline.
17. **Decide and implement practical anti-cheat hardening** — based on the attack campaign, choose which stock-mode hardening is justified: score-root commitment, always-bound `x_attn`, multi-point token/layer audit sampling, fail-closed missing-evidence behavior, local replay smoke, and offline canary/shadow-audit policy.
18. **Reduce commit/finalize overhead** — this is now the main online cost bottleneck after decode exactness and the audit-only stock attention path landed.
19. **Close the remaining trust/paper/docs language** — finish the paper revision, make the last-generated-token score-witness scope explicit, and lock the guarantee wording around audited attention inputs/wiring only.
20. **Take the next production-relevant frontier jump with DeepSeek-V3** — once the stock-mode claim, report surface, and audit policy are fully honest, start the DeepSeek-V3 track: MoE, MLA, and MTP.
...
2. **Stock mode has an honest attention contract** — arbitrary-position attention outputs are explicitly not verified; the kept audits are score anchoring, KV provenance, and wiring checks, with token-0/local replay smoke only. Audited-inputs E2E is green on Llama/Qwen audited profiles, and the current score-witness contract is explicitly last-generated-token only.
3. **Docs/guarantees match the kept single product path** — README, roadmap, benchmarks, and verifier reports already state the same claim: exact decode, audited attention inputs/wiring, no arbitrary-position attention verification. Remaining blocker: the paper still needs the same update.

### Current product / reference split

Two explicit buckets remain, but only one is a product commitment:

1. **Stock product mode** (kept) — keep vLLM's existing kernels and current answer semantics. On the decode side, the kept path is **CapturedLogits** for sampled decode: capture the actual GPU f32 logits, verify exact sampling, and Freivalds-bind back to `lp_hidden × lm_head_bf16`. For greedy decode, cheaper paths (i32 ExactTokenIdentity or LP-hidden bf16) remain acceptable where explicitly validated. On the attention side, the kept claim is narrower: exact score anchoring, KV provenance, mask/RoPE/GQA wiring audits, and token-0/local replay smoke checks. Current live score-witness scope is the last generated token. Arbitrary-position attention outputs are not verified.
...
9. ~~Rename the verifier/report surface to the kept stock-mode attention claim.~~ **DONE (2026-04-24).**
10. ~~Wire score anchoring, KV provenance, and wiring audits into the stock-mode audit path.~~ **DONE (2026-04-25).** Audited-inputs E2E is green on `llama-w8a8-audited` and `qwen-w8a8-audited`; score witnessing is currently last-generated-token only.
11. Keep token-0/local replay smoke as regression only; no product attention-verification claim.
12. Run the constrained stock-mode attention anti-cheat campaign (fake-`a`, sparse cheating, witness-scope evasion, local-replay evasion). **First pass landed (2026-04-26).** A0/A1 measured: 10/10 verifier passes, 4/10 undetected answer changes on consistent fake-`a` (toy bridge); A2–A5 deferred. See [`docs/security/redteam_audit_only.md`](./docs/security/redteam_audit_only.md).
13. Choose anti-cheat hardening based on those measurements.
14. Reduce commit/finalize overhead and finish the remaining honest product docs/paper.
15. Cut the release.
...
| 24a | **Stock-mode attention detectability campaign** — run constrained fake-`a` attacks against the shipped audit-only contract: single-layer cheating, sparse-layer cheating, early-vs-late cheating, last-token-witness evasion, and local-replay evasion. Report token-flip rate, detection rate by audit type, minimum perturbation needed to flip the accepted token, and detection probability under the actual sampled audit policy. This is the right empirical basis for claims like "harder to game" or "detectable" in stock mode. |
| 24b | **Anti-cheat hardening policy for stock-mode attention** — after 24a, explicitly choose which practical hardening is kept: score-root commitment, always-bound `x_attn`, multiple challenged tokens/layers, fail-closed missing evidence, local replay smoke, and offline canary/shadow-audit policy. Do not adopt these by intuition; tie them to the measured detectability results. |
...
| 68c | **GKR / sumcheck attention proof (audit-path, GPU prover)** — a not-yet-attempted path to actually *verify* arbitrary-position attention outputs, categorically different from the closed replay / `LSE` / `StockBounded` / deterministic-kernel lines: those are recompute/certification, this is an interactive proof. Prove the per-token attention circuit (`QK^T → softmax → αV`) with a sumcheck/GKR argument run **only on challenged tokens, on the prover's GPU**, off the serving path. Cost shape (Llama-70B back-of-envelope): circuit is tiny (`~16,384·n` MACs / layer / token); prover is bandwidth-bound on GPU (`~1 ms` routine / `~6 ms` full at 1K ctx; `~0.1 s` routine / `~0.8 s` full at 128K); proof is logarithmic (`~KB`); verifier ≈ reading the already-opened `Q/K/V` (one MLE eval), i.e. roughly the existing per-token CPU budget. Two design forks: **(a)** recompute softmax on the verifier from anchored scores (compute-light, but reintroduces the §3r score-witness payload wall, `41 MB–1.3 GB`); **(b)** softmax in-circuit via a lookup argument against a committed `exp` table, so GKR commits only to circuit inputs/outputs already retained (`Q/K/V`, `attn_out_i8`) → **near-zero extra payload**. Since GPU proving is cheap, prefer (b): trade prover cycles for payload. Caveats: proves the *quantized field reference* exactly (faithfulness to a committed deterministic reference, then tie to served fp16 via a quantization-only corridor, much narrower than §1i); needs a sumcheck-friendly field (BabyBear / Goldilocks + extension field for challenge soundness), separate from the `p = 2^32 − 5` Freivalds field; unlike verifier-secret Freivalds, a Fiat–Shamir GKR proof is **publicly verifiable**, so it could be the one genuinely transferable component of the receipt. Candidate stack (all Rust): `Expander` (Polyhedra, GPU GKR) or a custom engine over `Icicle` (GPU field/sumcheck kernels); `Lasso`-style lookups for the softmax `exp`; `Plonky3` if an AIR/STARK + LogUp shape is preferred over bare GKR. Status: design idea only, no code. |
