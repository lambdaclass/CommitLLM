...
The shell/tail/binding protocol is structurally complete. The maintained E2E target is now explicit: **exercise everything except arbitrary-position attention replay**. That means binary key/decode artifact, commitment-derived challenges, greedy and sampled decode, full shell/decode openings, routine/full coverage reporting, long prompts/generations, multi-position escalation, tamper rejection, and EOS behavior. Attention is excluded from random-position E2E audits by omitting KV openings; the only kept attention check in E2E is a token-0 smoke test. The Freivalds shell checks are gated by verification profile: Llama checks all 7 matrices (`Wq/Wk/Wv/Wo/Wg/Wu/Wd`); Qwen skips `Wq/Wk/Wv` because the bridge replay cannot match the GPU's quantized QKV GEMM. Token-identity acceptance is now cleanly split by decode mode: **sampled decode is shipped on both Llama and Qwen via `DecodeAcceptanceMode::CapturedLogits`**, which captures the exact GPU logits used by the sampler, verifies exact token sampling, and Freivalds-binds those logits back to `lp_hidden × lm_head_bf16`. GPU E2E is green on both models: Llama `23,335/23,335` decode checks passed, Qwen `17,336/17,336`. Greedy decode can still use cheaper validated paths (`ExactTokenIdentity` on Llama, `LpHiddenBf16` where useful) because argmax is robust to small replay differences. Decode economics are now partly measured and must be split into steady-state and escalation views: prover-side retained logits are ~`501 KiB/token` on Llama and ~`594 KiB/token` on Qwen (`~63/74 MiB` retained for 128 generated tokens, `~501/594 MiB` for 1024); full-tier opened payloads measured so far are on the order of `8–17 MiB` (Qwen) and `13–34 MiB` (Llama) depending on generation length, and current full-tier end-to-end verify time is about `~1.8s` (Qwen) and `~2.2s` (Llama). The capture A/B is now partly isolated: on Qwen, hooks add only about `~3.6 ms/token`, while commit/finalize adds about `~37 ms/token`; Llama still needs a clean capture rerun. Keygen for captured-logits profiles is now treated as an offline artifact-build cost (~`55s` Qwen, ~`152s` Llama), not steady-state verification. On the attention side, **the exact stock-kernel replay branch is now closed**: Llama f64 exact replay fails at non-zero positions; Qwen brute-force replay and fixed-tolerance witnessed-score replay are rejected; tiled online-softmax replay produced `delta = 0`; and captured `LSE` also failed to collapse the attention-output gap. The remaining stock-kernel path is therefore not exact replay but an explicitly weaker **stock-compatible practical tier**: exact score anchoring, tiny additional attention evidence where it materially helps, conservative tail bounds against committed `V`, and final-logit margin gating using exact `CapturedLogits`, with acceptance only when cheating cannot flip the output in practice. In parallel, the exact path becomes a separate **verified-attention mode** using deterministic attention kernels on the verified path; this mode may change answers relative to stock mode and must be documented as a distinct runtime/profile. The `lm_head` Freivalds matmul check remains exact for all profiles. The remaining open problems are: (1) **Correctness blocker — attention**: ship an honest stock-compatible practical tier and/or verified-attention mode, (2) **Cost blocker — captured-logits economics**: isolate capture overhead, retained footprint, open payload delta, and steady-state verifier time with cached keys/artifacts, (3) large decode-artifact distribution/storage (Qwen 7B `lm_head_bf16` is still ~1.57 GB), and (4) a production audit policy that clearly separates stock-compatible guarantees from verified-attention guarantees. **Decode policy**: sampled decode → CapturedLogits path (exact). Greedy decode → cheaper validated paths remain acceptable.
...
3. **Finish the captured-logits economics split** — baseline retained/open/full-tier numbers are now measured and Qwen hook-vs-commit overhead is partly isolated; the remaining gating question is the steady-state delta under cached key/artifact conditions. Finish the A/B benchmark (`capture off` vs `capture on`) to publish: (a) hook/copy overhead ms/token, (b) commit/finalize delta, (c) routine-tier payload + verify, (d) full-tier payload + verify, and (e) peak host memory. Treat keygen as an offline artifact build, not a steady-state cost.
4. **Freeze the two-mode dense-model architecture** — make the split explicit in profiles, docs, and policy: **stock-compatible mode** preserves the current runtime and answer semantics but only offers practical/approximate attention certification or unsupported attention; **verified-attention mode** uses deterministic attention kernels and may drift from stock answers.
5. **Build the stock-compatible practical attention tier** — exact stock-kernel replay is closed. The remaining stock-compatible path is a low-bandwidth non-hackability tier: exact score anchoring, tiny additional attention evidence only where it materially helps, conservative tail bounds against committed `V`, and final-logit margin gating using exact `CapturedLogits`.
6. **Measure stock-compatible certification rate and attacker room** — run broad randomized sweeps on Llama and Qwen, publish how often the practical tier certifies, how often it escalates/fails closed, and whether the bound is conservative enough to make output flips infeasible in practice.
7. **Freeze stock-compatible attention policy and escalation rules** — define which answers pass, which escalate to deeper audits or verified-attention mode, and which remain unsupported in stock-compatible mode.
8. **Write the deterministic-attention arithmetic spec for verified-attention mode** — narrow first slice: decode only, `seqlen_q = 1`, dense GQA, `head_dim = 128`, SM80/A100. Freeze accumulation order, cast points, softmax approximation, and output rounding before touching CUDA integration.
9. **Build the CPU reference for verified-attention mode** — canonical verifier-side implementation first.
10. **Build a standalone CUDA harness and require bit-exact CPU == GPU** — prove the arithmetic contract in isolation before vLLM integration.
11. **Integrate deterministic attention kernels as explicit verified-attention mode** — separate runtime/profile, not a silent replacement of stock mode.
...
2. **Stock-compatible mode has an honest attention contract** — either the practical low-bandwidth tier certifies that cheating cannot flip outputs in practice on broad randomized sweeps, or arbitrary-position attention is explicitly fail-closed / unsupported in stock-compatible mode.
...
2. **Verified-attention mode** (stronger / alternate runtime) — replace stock attention on the verified path with deterministic attention kernels. This is the exact-attention mode. It requires kernel engineering, a separate arithmetic contract, and a distinct runtime/profile because answers may drift from stock mode. Deterministic kernels mainly solve dense-model arithmetic: they make newer dense families and FP8-style dense serving much easier to support by restoring exact or near-exact GEMM semantics and removing most linear-layer tolerance tuning. They do **not** by themselves solve MoE routing, MLA / nonstandard KV state, speculative decoding / MTP semantics, sparse attention semantics, or cross-request cache semantics; those remain protocol/transcript work even if the arithmetic layer becomes exact.
...
4. Freeze the two-mode architecture: stock-compatible mode vs verified-attention mode.
5. Build the stock-compatible practical attention tier (`top-k` / tail bound / final-logit margin gating).
6. Measure certification rate, escalation rate, and attacker room on real prompts.
7. If stock-compatible certification is too weak, keep it narrow and move exact attention to verified-attention mode.
8. Keep exact attention replay as a reference/token-0 smoke tier, not production arbitrary-position acceptance.
...
11. Freeze the canonical verifier and update docs/claims.
12. Cut the release.
...
| 2 | **Shrink the honest corridor / land arbitrary-position attention verification** — prover-side determinism confirmed (§1h), sidecar-to-verifier gap measured at L∞=8–9 (§1i), but later experiments show that stock-kernel replay is not enough for production arbitrary-position attention claims. **Llama:** token-0 exact attention can pass, but non-zero positions can diverge heavily from FlashAttention/bf16; keep arbitrary-position E2E KV attention disabled until a better path exists. **Qwen:** brute-force replay fails at later positions, witnessed-score f64 softmax·V replay breached the proposed `±3` bound in the 39-prompt sweep, tiled replay showed `delta = 0`, and exact-GPU `LSE` replay also failed to collapse the gap. Backend binding is done (`attn_backend`/`attn_dtype` in manifest/key/verifier, SDPA mandated for W8A8, fail closed on eager/unknown). The next accepted paths are: (a) stock-compatible practical attention evidence (tiny attention witness, tail bounds, final-logit margin gating), or (b) verified-attention mode with deterministic attention kernels. | **blocked on practical attention tier / deterministic kernels** |
...
| 2h | **Strong-tier boundary policy** — for each supported family, specify which boundaries are: (a) captured exactly, (b) locally checked, (c) replayed approximately, (d) unsupported. Current policy until the stock-compatible practical attention tier or verified-attention mode lands: E2E random generated-token audits verify shell/decode and omit KV attention openings; Llama keeps token-0 exact-attention smoke only; arbitrary-position attention is either margin-bounded approximate (if the top-k/tail/margin path lands) or unsupported in stock-compatible mode; sampled decode is exact on both families via `CapturedLogits`; greedy decode may use cheaper validated replay paths where explicitly allowed. | partial |
...
| 8a | **Stock-compatible practical attention tier — ASAP** — exact stock-kernel replay is closed for Llama and Qwen: fixed-tolerance replay failed, tiled replay gave `delta = 0`, and exact-GPU-`LSE` replay also failed to collapse the gap. The remaining stock-compatible path is a practical non-hackability tier for **decode-time attention only** (`seqlen_q = 1`) on the actual vLLM/FlashAttention-2 path. Sequence of work: **Step 1** — capture tiny high-value attention evidence that directly constrains attacker freedom (start with top-k indices/weights or equivalent small witness, not more replay-normalization work); **Step 2** — bound the unseen tail against committed `V`; **Step 3** — propagate the worst-case attention error through the exact downstream shell; **Step 4** — use exact final-token margins from `CapturedLogits` to accept only when the maximum possible attention error cannot flip the output token; **Step 5** — define clear accept / escalate / unsupported rules for stock-compatible mode. Hard promotion criteria: broad randomized prompts, low attacker-success rate under the bound, payload stays in the KB-to-low-hundreds-of-KB range per challenged token, and clear escalation to verified-attention mode when the bound is too loose. If this does not produce a usable acceptance region, keep stock-compatible mode narrow and promote deterministic attention kernels as the exact verified-attention path. | **ASAP / open** |
...
| 10 | **Deep-audit payload sizing / policy** — budget the per-token cost of the current shell/decode E2E surface separately from future attention evidence. Current practical policy: random generated-token audits open full shell/decode with `include_kv=false`; token-0 attention smoke is allowed; arbitrary-position attention remains unsupported until the stock-compatible practical tier or verified-attention mode lands. Current measured decode-side economics: retained sampled-decode logits are ~501 KiB/token (Llama) / ~594 KiB/token (Qwen); measured full-tier payloads are ~13–34 MiB (Llama) / ~8–17 MiB (Qwen); measured full-tier end-to-end verify is ~2.2s (Llama) / ~1.8s (Qwen); completed Qwen A/B shows hook overhead is modest while commit/finalize dominates. Remaining required split: routine vs full payload/verify and a clean Llama capture A/B. Future sizing should evaluate top-k / tail-bound payloads for stock-compatible mode first, then deterministic-kernel costs for verified-attention mode — not wider softmax·V replay tolerances. **Must benchmark CPU verification time in the Rust verifier**, not estimate from FLOPs. | open |
...
| 99 | Update docs with the kept dense-model story: sampled decode exact via `CapturedLogits`; the product has two explicit modes (`stock-compatible` and `verified-attention`); stock-compatible arbitrary-position attention is either margin-bounded approximate via the practical tier or unsupported; verified-attention mode uses deterministic kernels and may drift from stock answers; measured decode economics are separated into retained/open/routine/full costs; no stale replay-tolerance language |
