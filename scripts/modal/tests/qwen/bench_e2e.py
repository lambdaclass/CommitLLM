"""
Qwen 2.5 7B W8A8 — shell/decode benchmark.
...
arbitrary-position attention):
  ...
  - Greedy vs sampled decode (LP-hidden bf16 path)
...
Usage:
    modal run --detach scripts/modal/tests/qwen/bench_e2e.py
"""
...
app = modal.App("commitllm-bench-qwen-e2e")
...
def _run():
    ...
    verifier_secret = hashlib.sha256(b"bench-qwen-e2e-secret").digest()
    t0 = time.time()
...
def main():
    print("CommitLLM Qwen E2E Benchmark")
    print("=" * 60)
