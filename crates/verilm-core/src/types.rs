...
pub struct V4AuditResponse {
    ...
    /// Replayed full layer traces for the challenged token.
    ///
    /// Generated by the prover at audit time by rerunning computation
    /// from retained state + public weights. The verifier checks:
    /// 1. Retained-state binding: replayed `a` must match retained `a`
    /// 2. Freivalds on all 7 matmuls per layer
    /// 3. SiLU, intra-layer chain, cross-layer chain
    ///
    /// `None` for structural-only verification (Merkle + IO chain + bindings).
    #[serde(default)]
    pub replayed_layers: Option<Vec<LayerTrace>>,
}
