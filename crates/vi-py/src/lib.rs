...
//! to build commitments and generate proofs without going through vi-server.
//!
//! Usage from Python:
//! ```python
//! import verilm_rs
...
fn verify_batch<'py>(
    ...
    let report =
        vi_verify::verify_batch_with_policy(&key, &proof, seed_bytes, challenge_k, &policy_obj);

    let result = PyDict::new(py);
    result.set_item("passed", report.verdict == vi_verify::Verdict::Pass)?;
