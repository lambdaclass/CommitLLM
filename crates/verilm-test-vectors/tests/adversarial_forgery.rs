...
use verilm_core::types::{BatchProof, TokenTrace};
use verilm_test_vectors::*;
use verilm_verify::{verify_trace_at_level, Verdict, VerificationLevel};
...
mod cross_token_replay {
    ...
    fn test_replay_entire_trace_wrong_position() {
        ...
        let (passed, failures) = verilm_test_vectors::verify_batch(&key, &proof, &challenges);
...
mod spliced_traces {
    ...
    fn test_splice_layer0_from_run_a_rest_from_run_b() {
        ...
        if expected_next_a != layers_b[1].x_attn {
            ...
            let (passed, failures) = verilm_test_vectors::verify_trace(&key, &trace);
            ...
            let spliced = vec![layers_a[0].clone(), layers_b[1].clone()];
            let trace = build_trace(spliced, 0, 10);
            let (passed, _) = verilm_test_vectors::verify_trace(&key, &trace);
