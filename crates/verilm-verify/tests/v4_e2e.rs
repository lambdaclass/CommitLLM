...
use verilm_prover::{
    commit_minimal, open_v4, open_v4_structural, CapturedLayerScales, FullBindingParams,
};
use verilm_test_vectors::{forward_pass, generate_key, generate_model, LayerWeights};
use verilm_verify::{
    verify_v4_legacy, verify_v4_with_weights, Detokenizer, PromptTokenizer, Verdict,
};
...
fn commit_toy(
    ...
    verilm_prover::MinimalBatchState,
) {
    let n_tokens = all_retained.len();
    let scales = vec![unit_scales(n_layers); n_tokens];
    commit_minimal(all_retained, params, final_residuals, scales, None, None, None, None)
}
...
fn v4_tampered_io_chain_detected() {
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("prev_io_hash") || f.message.contains("IO chain")),
...
fn retained_with_scales(
    traces: &[verilm_core::types::LayerTrace],
...
fn v4_scale_aware_single_token_pass() {
    ...
    let (_commitment, state) =
        commit_minimal(vec![retained], &params, None, vec![rscales], None, None, None, None);
    let response = open_v4(
        &state,
        0,
        &ToyWeights(&model),
...
fn v4_scale_mismatch_detected() {
    ...
    assert_eq!(
        report.verdict,
        Verdict::Fail,
        "scale mismatch should cause failure"
    );
    assert!(
        report
            .failures
...
fn v4_full_bridge_cross_layer_chain() {
    ...
    let (all_retained, all_scales): (Vec<RetainedTokenState>, Vec<Vec<CapturedLayerScales>>) =
        residuals
            .iter()
            .map(|ir| {
                full_bridge_forward(
...
fn v4_full_bridge_wrong_residual_detected() {
    ...
    assert_eq!(
        report.verdict,
        Verdict::Fail,
        "wrong residual should be detected"
    );
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("embedding Merkle proof")),
...
fn v4_full_bridge_qkv_layer0() {
    ...
    assert!(
        ...
        "full bridge should produce QKV at layer 0"
    );
...
fn v4_embedding_proof_tampered_residual_detected() {
    ...
    assert_eq!(
        report.verdict,
        Verdict::Fail,
        "tampered residual should be caught"
    );
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("embedding Merkle proof")),
        "should fail on embedding proof, failures: {:?}",
        report.failures
    );
}
...
fn v4_embedding_proof_missing_when_root_present() {
    ...
    assert_eq!(
        report.verdict,
        Verdict::Fail,
        "missing proof should be caught"
    );
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("missing embedding_proof")),
        "should fail on missing proof, failures: {:?}",
        report.failures
    );
}
...
fn v4_embedding_proof_wrong_token_id_detected() {
    ...
    assert_eq!(
        report.verdict,
        Verdict::Fail,
        "wrong token_id should be caught"
    );
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("leaf_index") && f.message.contains("token_id")),
        "should fail on token_id mismatch, failures: {:?}",
        report.failures
    );
}
...
fn v4_downgrade_omit_initial_residual_detected() {
    ...
    assert_eq!(
        report.verdict,
        Verdict::Fail,
        "omitted initial_residual should be caught"
    );
    assert!(
        report
            .failures
        ...
        "should fail on missing initial_residual, failures: {:?}",
        report.failures
    );
}
...
fn v4_unbound_initial_residual_rejected() {
    ...
    let scales = bridge_scales(&cfg);
    assert!(
        key.embedding_merkle_root.is_none(),
        "key should have no embedding root"
    );
    ...
    assert_eq!(
        report.verdict,
        Verdict::Fail,
        "unbound initial_residual should be rejected"
    );
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("embedding_merkle_root")),
        "should fail on missing embedding root, failures: {:?}",
        report.failures
    );
}
...
fn v4_lm_head_wrong_token_detected() {
    ...
    assert_eq!(
        report.verdict,
        Verdict::Fail,
        "wrong token should be detected"
    );
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("lm_head")),
        "should fail on lm_head check, failures: {:?}",
        report.failures
    );
}
...
fn v4_lm_head_freivalds_catches_tampered_logits() {
    ...
    assert!(
        !key.r_for(MatrixType::LmHead).is_empty(),
        "r for LmHead should be generated"
    );
    ...
    assert_eq!(
        report.verdict,
        Verdict::Fail,
        "tampered logits should be caught"
    );
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("Freivalds")),
        "should fail on Freivalds check, got: {:?}",
        report.failures
    );
}
...
fn v4_manifest_wrong_sampled_token_detected() {
    ...
    assert!(
        report
            ...
            .iter()
            .any(|f| f.message.contains("lm_head")),
        "should fail on lm_head sampling check, failures: {:?}",
        report.failures
    );
}
...
fn v4_manifest_hash_mismatch_detected() {
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("manifest hash")),
        "should fail on manifest hash, failures: {:?}",
        report.failures
    );
}
...
fn v4_manifest_n_layers_mismatch_rejected() {
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("n_layers mismatch")),
        "expected n_layers mismatch, got: {:?}",
        report.failures
    );
}
...
fn v4_manifest_hidden_dim_mismatch_rejected() {
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("hidden_dim mismatch")),
...
fn v4_manifest_vocab_size_mismatch_rejected() {
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("vocab_size mismatch")),
        "expected vocab_size mismatch, got: {:?}",
        report.failures
    );
}
...
fn v4_manifest_embedding_root_mismatch_rejected() {
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("embedding_merkle_root mismatch")),
        "expected embedding_merkle_root mismatch, got: {:?}",
        report.failures
    );
}
...
fn v4_manifest_architecture_fields_pass() {
    ...
    assert!(
        report.checks_run >= 11,
        "expected extra architecture checks, got {}",
        report.checks_run
    );
}
...
fn v4_decode_mode_greedy_with_nonzero_temp_rejected() {
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("decode_mode='greedy'")),
        "expected decode_mode/temperature mismatch, got: {:?}",
        report.failures
    );
}
...
fn v4_decode_mode_sampled_with_zero_temp_rejected() {
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("decode_mode='sampled'")),
        "expected decode_mode/temperature mismatch, got: {:?}",
        report.failures
    );
}
...
fn v4_decode_mode_unknown_rejected() {
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("unsupported decode_mode")),
        "expected unsupported decode_mode, got: {:?}",
        report.failures
    );
}
...
fn v4_manifest_quant_family_mismatch_rejected() {
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("quant_family mismatch")),
        "expected quant_family mismatch, got: {:?}",
        report.failures
    );
}
...
fn v4_manifest_scale_derivation_mismatch_rejected() {
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("scale_derivation mismatch")),
        "expected scale_derivation mismatch, got: {:?}",
        report.failures
    );
}
...
fn v4_manifest_quant_block_size_mismatch_rejected() {
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("quant_block_size mismatch")),
        "expected quant_block_size mismatch, got: {:?}",
        report.failures
    );
}
...
fn v4_manifest_kv_dim_mismatch_rejected() {
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("kv_dim mismatch")),
        "expected kv_dim mismatch, got: {:?}",
        report.failures
    );
}
...
fn v4_manifest_ffn_dim_mismatch_rejected() {
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("ffn_dim mismatch")),
        "expected ffn_dim mismatch, got: {:?}",
        report.failures
    );
}
...
fn v4_manifest_d_head_mismatch_rejected() {
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("d_head mismatch")),
        "expected d_head mismatch, got: {:?}",
        report.failures
    );
}
...
fn v4_manifest_n_q_heads_mismatch_rejected() {
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("n_q_heads mismatch")),
        "expected n_q_heads mismatch, got: {:?}",
        report.failures
    );
}
...
fn v4_manifest_n_kv_heads_mismatch_rejected() {
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("n_kv_heads mismatch")),
        "expected n_kv_heads mismatch, got: {:?}",
        report.failures
    );
}
...
fn v4_manifest_rope_theta_mismatch_rejected() {
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("rope_theta mismatch")),
        "expected rope_theta mismatch, got: {:?}",
        report.failures
    );
}
...
fn v4_manifest_full_architecture_pass() {
    ...
    assert!(
        report.checks_run >= 17,
        "expected full architecture checks, got {}",
        report.checks_run
    );
}
...
fn attach_tail_logits(
    ...
    let fr = shell
        .final_residual
        .as_ref()
        .expect("final_residual required for tail logits");
...
fn setup_final_residual() -> (
    ...
    let normed = verilm_core::rmsnorm::rmsnorm_f64_input(&res_f64, &final_norm_weights, 1e-5);
    let final_hidden: Vec<i8> = normed
        .iter()
        .map(|&v| v.round().clamp(-128.0, 127.0) as i8)
        .collect();
    let logits = verilm_core::sampling::recompute_logits(
...
fn v4_captured_final_residual_tampered_residual_detected() {
    ...
    assert_eq!(
        report.verdict,
        Verdict::Fail,
        "tampered residual should be detected, failures: {:?}",
        report.failures
    );
    assert!(
        report
...
fn v4_final_residual_fail_closed_missing() {
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("final_residual")),
        "should mention final_residual in failure, failures: {:?}",
        report.failures
    );
}
...
fn v4_lm_head_fail_closed_missing_logits() {
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("logits_i32")),
        "should mention logits_i32 in failure, failures: {:?}",
        report.failures
    );
}
...
fn v4_final_residual_commitment_binding() {
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("Merkle proof failed")),
        "should fail on Merkle proof, failures: {:?}",
        report.failures
    );
}
...
fn verify_with_manifest(manifest: DeploymentManifest) -> verilm_verify::V4VerifyReport {
    ...
    let (_commitment, state) = commit_toy(
        vec![retained_from_traces(&traces)],
        &params,
        None,
        cfg.n_layers,
    );
...
fn v4_manifest_rejects_repetition_penalty() {
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("repetition_penalty")),
        "expected repetition_penalty rejection, got: {:?}",
        report.failures
    );
}
...
fn v4_manifest_rejects_frequency_penalty() {
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("frequency_penalty")),
        "expected frequency_penalty rejection, got: {:?}",
        report.failures
    );
}
...
fn v4_manifest_rejects_presence_penalty() {
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("presence_penalty")),
        "expected presence_penalty rejection, got: {:?}",
        report.failures
    );
}
...
fn v4_manifest_rejects_logit_bias() {
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("logit_bias")),
        "expected logit_bias rejection, got: {:?}",
        report.failures
    );
}
...
fn v4_manifest_rejects_bad_word_ids() {
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("bad_word_ids")),
        "expected bad_word_ids rejection, got: {:?}",
        report.failures
    );
}
...
fn v4_manifest_rejects_guided_decoding() {
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("guided_decoding")),
        "expected guided_decoding rejection, got: {:?}",
        report.failures
    );
}
...
fn v4_manifest_rejects_stop_sequences() {
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("stop_sequences")),
        "expected stop_sequences rejection, got: {:?}",
        report.failures
    );
}
...
fn v4_manifest_rejects_exceeded_max_tokens() {
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("max_tokens")),
        "expected max_tokens rejection, got: {:?}",
        report.failures
    );
}
...
fn v4_manifest_rejects_overlong_transcript() {
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("generated") && f.message.contains("max_tokens")),
        "expected generation-length max_tokens rejection, got: {:?}",
        report.failures
    );
}
...
fn v4_manifest_max_tokens_zero_means_unlimited() {
    ...
    assert_eq!(
        report.verdict,
        Verdict::Pass,
        "max_tokens=0 (unlimited) should pass, failures: {:?}",
        report.failures
    );
}
...
fn v4_manifest_accepts_all_defaults() {
    ...
    assert_eq!(
        report.verdict,
        Verdict::Pass,
        "canonical defaults should pass, failures: {:?}",
        report.failures
    );
}
...
fn v4_manifest_rejects_missing_spec_hashes() {
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("missing input_spec_hash")),
        "should fail on missing input_spec_hash, failures: {:?}",
        report.failures
    );
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("missing model_spec_hash")),
        "should fail on missing model_spec_hash, failures: {:?}",
        report.failures
    );
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("missing decode_spec_hash")),
        "should fail on missing decode_spec_hash, failures: {:?}",
        report.failures
    );
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("missing output_spec_hash")),
        "should fail on missing output_spec_hash, failures: {:?}",
        report.failures
    );
}
...
fn v4_rmsnorm_eps_mismatch_detected() {
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("rmsnorm_eps mismatch")),
        "expected rmsnorm_eps mismatch, got: {:?}",
        report.failures
    );
}
...
fn v4_rmsnorm_eps_matching_passes() {
    ...
    assert_eq!(
        report.verdict,
        Verdict::Pass,
        "matching rmsnorm_eps should pass: {:?}",
        report.failures
    );
}
...
fn v4_rope_config_hash_mismatch_detected() {
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("rope_config_hash mismatch")),
        "expected rope_config_hash mismatch, got: {:?}",
        report.failures
    );
}
...
fn v4_weight_hash_mismatch_detected() {
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("weight_hash mismatch")),
        "expected weight_hash mismatch, got: {:?}",
        report.failures
    );
}
...
fn v4_sampler_version_unknown_rejected() {
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("unsupported sampler_version")),
        "expected sampler_version rejection, got: {:?}",
        report.failures
    );
}
...
fn v4_sampler_version_canonical_passes() {
    ...
    assert_eq!(
        report.verdict,
        Verdict::Pass,
        "canonical sampler_version should pass: {:?}",
        report.failures
    );
}
...
fn v4_prompt_hash_binding() {
    ...
    assert_eq!(
        response.prompt.as_deref(),
        Some(b"the real prompt".as_slice())
    );
    ...
    assert!(
        !report
            .failures
            .iter()
            .any(|f| f.message.contains("prompt_hash")),
        "prompt hash should verify, failures: {:?}",
        report.failures
    );
}
...
fn v4_prompt_hash_tamper_detected() {
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("prompt_hash mismatch")),
        "should detect prompt tampering, failures: {:?}",
        report.failures
    );
}
...
fn v4_verify_input_tokenization_mismatch() {
    ...
    assert!(
        failures[0].message.contains("prompt token mismatch"),
        "failure: {}",
        failures[0]
    );
}
...
fn v4_cross_request_splice_shell_opening() {
    ...
    assert_eq!(
        report.verdict,
        Verdict::Fail,
        "cross-request splice must be detected, got: {:?}",
        report.failures
    );
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("Merkle proof")),
        "expected Merkle proof failure from spliced retained state, got: {:?}",
        report.failures
    );
}
...
fn v4_cross_request_splice_with_manifest() {
    ...
    assert_eq!(
        report.verdict,
        Verdict::Fail,
        "cross-request splice with manifest must be detected, got: {:?}",
        report.failures
    );
    ...
    assert!(
        report
            ...
            .iter()
            .any(|f| f.message.contains("manifest hash")),
        "expected manifest hash mismatch, got: {:?}",
        report.failures
    );
    ...
    assert!(
        report
            ...
            .iter()
            .any(|f| f.message.contains("Merkle proof")),
        "expected Merkle proof failure, got: {:?}",
        report.failures
    );
}
...
fn v4_cross_request_splice_token_id_swap() {
    ...
    let inputs_b: Vec<Vec<i8>> = (0..3)
        .map(|t| {
            (0..cfg.hidden_dim)
                .map(|i| ((i * 11 + t * 31 + 80) % 256) as i8)
                .collect()
        })
        .collect();

    let all_retained_a: Vec<RetainedTokenState> = inputs_a
        .iter()
        .map(|inp| retained_from_traces(&forward_pass(&cfg, &model, inp)))
        .collect();
    let all_retained_b: Vec<RetainedTokenState> = inputs_b
        .iter()
        .map(|inp| retained_from_traces(&forward_pass(&cfg, &model, inp)))
        .collect();
    ...
    assert_eq!(
        report.verdict,
        Verdict::Fail,
        "cross-request token splice must be detected, got: {:?}",
        report.failures
    );
    assert!(
        report
            ...
            .iter()
            .any(|f| f.message.contains("Merkle proof")
                || f.message.contains("IO")
                || f.message.contains("chain")),
        "expected Merkle or IO chain failure, got: {:?}",
        report.failures
    );
}
...
fn v4_integrated_tokenization_mismatch_detected() {
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("prompt token mismatch")),
        "should detect mismatch, failures: {:?}",
        report.failures
    );
}
...
fn v4_min_tokens_rejects_early_eos() {
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("min_tokens")),
        "should reject early EOS, failures: {:?}",
        report.failures
    );
}
...
fn v4_min_tokens_rejects_short_generation() {
    ...
    assert!(
        report
            ...
            .iter()
            .any(|f| f.message.contains("min_tokens")),
        "should reject short generation, failures: {:?}",
        report.failures
    );
}
...
fn v4_ignore_eos_rejects_eos_token() {
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("ignore_eos")),
        "should reject EOS when ignore_eos=true, failures: {:?}",
        report.failures
    );
}
...
fn v4_output_policy_fails_closed_without_eos_token_id() {
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("eos_token_id is missing")),
        "should fail-closed without eos_token_id, failures: {:?}",
        report.failures
    );
}
...
fn v4_eos_policy_stop_allows_eos_as_last_token() {
    ...
    assert!(
        !report
            .failures
            .iter()
            .any(|f| f.message.contains("eos_policy")),
        "eos_policy should not fail when EOS is last token, failures: {:?}",
        report.failures
    );
}
...
fn v4_eos_policy_stop_rejects_eos_mid_sequence() {
    ...
    let (cfg, model, key, _lm_head, input, token_id) = setup_lm_head();
    let input2: Vec<i8> = (0..cfg.hidden_dim)
        .map(|i| ((i * 3 + 50) % 256) as i8)
        .collect();
    ...
    assert!(
        report
            ...
            .iter()
            .any(|f| f.message.contains("eos_policy")),
        "should reject EOS mid-sequence with stop policy, failures: {:?}",
        report.failures
    );
}
...
fn v4_unknown_eos_policy_rejected() {
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("unknown eos_policy")),
        "should reject unknown eos_policy, failures: {:?}",
        report.failures
    );
}
...
fn v4_tokenizer_trait_reconstruction_pass() {
    ...
    let tokenizer = FixedTokenizer {
        token_ids: vec![100, token_id],
    };
    let report = verify_v4_full(
        &key,
        &response,
        None,
        Some(&tokenizer as &dyn PromptTokenizer),
        None,
    );
    assert_eq!(
        report.verdict,
...
fn v4_tokenizer_trait_reconstruction_mismatch() {
    ...
    assert!(
        report
            ...
            .iter()
            .any(|f| f.message.contains("prompt token mismatch")),
        "should detect mismatch from reconstructed tokens, failures: {:?}",
        report.failures
    );
}
...
fn v4_tokenizer_trait_error_reported() {
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("tokenizer reconstruction failed")),
        "should report tokenizer error, failures: {:?}",
        report.failures
    );
}
...
fn v4_tokenizer_fallback_to_caller_supplied() {
    ...
    assert_eq!(
        report.verdict,
        Verdict::Pass,
        "should fall back to caller-supplied IDs, failures: {:?}",
        report.failures
    );
}
...
fn v4_detokenization_mismatch_detected() {
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("detokenization mismatch")),
        "expected detokenization mismatch, got: {:?}",
        report.failures
    );
}
...
fn v4_detokenization_error_reported() {
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("detokenization failed")),
        "expected detokenization failure, got: {:?}",
        report.failures
    );
}
...
fn v4_detokenization_fails_closed_without_output_text() {
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("missing output_text")),
        "expected fail-closed for missing output_text, got: {:?}",
        report.failures
    );
}
...
fn v4_detokenization_not_checked_without_detokenizer() {
    ...
    let report = verify_v4_full(&key, &response, None, None, no_detok);
    assert_eq!(
        report.verdict,
        Verdict::Pass,
        "failures: {:?}",
...
fn v4_rich_prefix_embedding_pass() {
    ...
    let lookup = build_embedding_lookup_for_tokens(
        n_vocab,
        cfg.hidden_dim,
        &[
            (10, &residuals[0]),
    ...
    assert!(
        response.prefix_embedding_rows.is_some(),
        "should have prefix embedding rows"
    );
    assert!(
        response.prefix_embedding_proofs.is_some(),
        "should have prefix embedding proofs"
    );
    ...
    assert!(
        report.checks_run >= 10 + cfg.n_layers * 7,
...
fn v4_rich_prefix_tampered_embedding_detected() {
    ...
    assert_eq!(
        report.verdict,
        Verdict::Fail,
        "tampered prefix embedding should be caught"
    );
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("prefix token 0") && f.message.contains("Merkle proof")),
        "should fail on prefix embedding Merkle proof, got: {:?}",
        report.failures
    );
}
...
fn v4_compact_prefix_still_works() {
    ...
    let response = open_v4(
        ...
        None,
        false,
        false, // no embedding_lookup → compact
    );

    assert!(
        response.prefix_embedding_rows.is_none(),
        "compact mode should not have prefix rows"
    );
    assert!(
        response.prefix_embedding_proofs.is_none(),
        "compact mode should not have prefix proofs"
    );
    ...
    assert_eq!(
        report.verdict,
        Verdict::Pass,
        "compact prefix should still pass: {:?}",
        report.failures
    );
}
...
fn v4_deep_prefix_pass() {
    ...
    assert!(
        ...
        "should have prefix retained"
    );
    assert!(
        response.prefix_shell_openings.is_some(),
        "should have prefix shell openings"
    );
    ...
    assert!(
        report.checks_run >= min_expected,
        "expected >= {} checks for deep prefix, got {}",
        min_expected,
        report.checks_run
    );
}
...
fn v4_deep_prefix_tampered_retained_detected() {
    ...
    assert_eq!(
        report.verdict,
        Verdict::Fail,
        "tampered retained should be caught"
    );
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("prefix token 0") && f.message.contains("retained hash")),
        "should fail on retained hash mismatch, got: {:?}",
        report.failures
    );
}
...
fn v4_deep_prefix_tampered_shell_detected() {
    ...
    assert_eq!(
        report.verdict,
        Verdict::Fail,
        "tampered shell accumulator should be caught"
    );
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("prefix token 0") && f.message.contains("Freivalds")),
        "should fail on Freivalds for prefix token, got: {:?}",
        report.failures
    );
}
...
fn v4_deep_prefix_compact_mode_unaffected() {
    ...
    assert!(
        response.prefix_embedding_rows.is_some(),
        "Tier A should be present"
    );
    assert!(
        response.prefix_retained.is_none(),
        "Tier B should be absent"
    );
    assert!(
        response.prefix_shell_openings.is_none(),
        "Tier B should be absent"
    );
...
fn v4_deep_prefix_count_mismatch_rejected() {
    ...
    assert_eq!(
        report.verdict,
        Verdict::Fail,
        "count mismatch should be caught"
    );
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.message.contains("deep prefix count mismatch")),
        "should fail on count mismatch, got: {:?}",
        report.failures
    );
}
...
fn v4_full_audit_reports_full_coverage() {
    ...
    match &report.coverage {
        verilm_verify::AuditCoverage::Full { layers_checked } => {
            assert_eq!(
                *layers_checked, cfg.n_layers,
                "full audit should check all {} layers",
                cfg.n_layers
            );
        }
...
fn v4_routine_audit_reports_routine_coverage() {
    ...
    assert!(
        cfg.n_layers >= 2,
        "need at least 2 layers for routine-audit test"
    );
...
fn v4_non_contiguous_layer_indices_rejected() {
    ...
    assert!(
        report
            .failures
            .iter()
            .any(|f| f.code == verilm_verify::FailureCode::NonContiguousLayerIndices),
        "should reject non-contiguous layer_indices, got: {:?}",
        report.failures
    );
}
...
fn v4_pass_display_includes_coverage() {
    ...
    assert!(
        display.contains("coverage: full"),
        "display should include coverage, got: {}",
        display
    );
}
...
fn v4_routine_audit_not_mistaken_for_full() {
    ...
    let mut response_routine = open_v4(
        &state,
        0,
        &ToyWeights(&model),
        &cfg,
    ...
    assert_ne!(
        report_full.coverage, report_routine.coverage,
