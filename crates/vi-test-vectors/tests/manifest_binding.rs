...
use vi_core::merkle;
use vi_core::types::DeploymentManifest;
use vi_test_vectors::*;

fn toy_config() -> vi_core::constants::ModelConfig {
    vi_core::constants::ModelConfig {
...
fn toy_all_layers() -> Vec<Vec<vi_core::types::LayerTrace>> {
    let cfg = toy_config();
...
fn test_honest_manifest_binding_passes() {
    ...
    let failures = vi_verify::verify_manifest(&commitment, &manifest);
...
fn test_wrong_temperature_detected() {
    ...
    let failures = vi_verify::verify_manifest(&commitment, &wrong);
