...
fn test_corrupt_each_matrix_type_individually() {
    ...
    let matrix_fields: Vec<(&str, Box<dyn Fn(&mut vi_core::types::LayerTrace)>)> = vec![
        ("Wq", Box::new(|lt: &mut vi_core::types::LayerTrace| lt.q[0] += 1)),
...
fn test_single_bit_flip_in_every_output_field() {
    ...
    let fields: Vec<(&str, Box<dyn Fn(&mut vi_core::types::LayerTrace)>)> = vec![
        ("q", Box::new(|lt: &mut vi_core::types::LayerTrace| lt.q[0] ^= 1)),
...
fn test_cross_token_coordinated_corruption() {
    ...
    let proof = vi_core::types::BatchProof {
...
fn test_chain_preserving_local_forgery() {
    ...
    let new_h = vi_core::silu::compute_h_unit_scale(&layers[0].g, &layers[0].u);
    layers[0].h = new_h;
...
fn test_silu_plus_chain_combined_attack() {
    ...
    let forged_h = vi_core::silu::compute_h_unit_scale(&layers[0].g, &layers[0].u);
    layers[0].h = forged_h;
...
fn test_silu_forgery_with_consistent_h() {
    ...
    let honest_h = vi_core::silu::compute_h_unit_scale(&layers[0].g, &layers[0].u);
