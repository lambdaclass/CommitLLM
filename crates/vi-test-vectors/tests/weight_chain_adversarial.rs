...
use vi_core::merkle::hash_weights;
...
use vi_verify::WeightHashResult;
...
fn test_end_to_end_published_checkpoint_binding() {
    ...
    let traces = vi_test_vectors::forward_pass(&cfg, &model, &input);
    let trace = vi_test_vectors::build_trace(traces, 0, 1);

    let result = vi_verify::verify_trace(&key, &trace);
    assert_eq!(result.verdict, vi_verify::Verdict::Pass, "honest trace must pass");
}
...
fn test_end_to_end_trace_passes_but_wrong_model_detected_at_binding() {
    ...
    let input = vec![1i8; cfg.hidden_dim];
    let traces = vi_test_vectors::forward_pass(&cfg, &model_evil, &input);
    let trace = vi_test_vectors::build_trace(traces, 0, 1);

    // The trace passes Freivalds (it WAS computed correctly with evil model)
    let result = vi_verify::verify_trace(&key_evil, &trace);
    assert_eq!(result.verdict, vi_verify::Verdict::Pass, "trace is honestly computed, Freivalds passes");
...
fn test_verify_weight_hash_match() {
    ...
    let result = vi_verify::verify_weight_hash(&key, Some(&expected));
...
fn test_verify_weight_hash_mismatch() {
    ...
    let result = vi_verify::verify_weight_hash(&key, Some(&wrong_expected));
    match result {
...
fn test_verify_weight_hash_skipped_when_none_expected() {
    ...
    let result = vi_verify::verify_weight_hash(&key, None);
...
fn test_check_single_trace_with_correct_expected_hash() {
    ...
    assert_eq!(
        vi_verify::verify_weight_hash(&key, Some(&expected)),
        WeightHashResult::Match
    );
    // Trace also passes
    let report = vi_verify::verify_trace(&key, &trace);
    assert_eq!(report.verdict, vi_verify::Verdict::Pass);
}
...
fn test_check_single_trace_with_wrong_expected_hash_rejects() {
    ...
    match vi_verify::verify_weight_hash(&key, Some(&wrong)) {
...
fn test_check_batch_with_correct_expected_hash() {
    ...
    let seed = [42u8; 32];
    let challenge_k = 3;
    let challenges = vi_core::merkle::derive_challenges(
    ...
    let report = vi_verify::verify_batch(&key, &proof, seed, challenge_k);
    assert_eq!(report.verdict, vi_verify::Verdict::Pass);
}
...
fn test_strict_mode_legacy_key_rejected_with_any_expected_hash() {
    ...
    assert_eq!(
        vi_verify::verify_weight_hash(&stripped_key, Some(&correct_expected)),
        WeightHashResult::LegacyKeyNoHash,
    );
}
...
fn test_strict_mode_legacy_key_passes_without_expected_hash() {
    ...
    assert_eq!(
        vi_verify::verify_weight_hash(&key, None),
...
fn test_strict_mode_after_serialization_roundtrip() {
    ...
    assert_eq!(
        vi_verify::verify_weight_hash(&key2, Some(&correct_expected)),
        WeightHashResult::LegacyKeyNoHash,
    );
}
...
fn test_fixture_recompute_published_hash_matches_keygen() {
    ...
    assert_eq!(
        vi_verify::verify_weight_hash(&key, Some(&published_hash)),
        WeightHashResult::Match,
    ...
    let report = vi_verify::verify_trace(&key, &trace);
    assert_eq!(report.verdict, vi_verify::Verdict::Pass, "honest trace must pass Freivalds");
}
...
fn test_fixture_recompute_detects_checkpoint_mismatch() {
    ...
    match vi_verify::verify_weight_hash(&evil_key, Some(&published_hash)) {
        WeightHashResult::Mismatch { key_hash, expected } => {
...
fn test_fixture_batch_end_to_end_with_published_hash() {
    ...
    let report = vi_verify::verify_batch(&key, &proof, seed, 4);
    assert_eq!(report.verdict, vi_verify::Verdict::Pass);
}
