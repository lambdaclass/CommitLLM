...
use vi_core::attention::{AttentionToleranceConfig, compare_attention_output, replay_attention_reference};
use vi_core::constants::ModelConfig;
use vi_core::margin::{build_margin_certificate, DEFAULT_ATTENTION_EPSILON};
use vi_test_vectors::*;
use vi_verify::VerificationLevel;
...
fn test_single_token_level_c_pass() {
    ...
    let report = vi_verify::verify_trace_at_level(&key, &trace, VerificationLevel::C, DEFAULT_ATTENTION_EPSILON);
    assert_eq!(report.verdict, vi_verify::Verdict::Pass, "failures: {:?}", report.failures);
}
...
fn test_tampered_attention_detected_level_c() {
    ...
    let report = vi_verify::verify_trace_at_level(&key, &trace, VerificationLevel::C, DEFAULT_ATTENTION_EPSILON);
    assert_eq!(report.verdict, vi_verify::Verdict::Fail);
    assert!(
...
fn test_configurable_tolerance_level_c() {
    ...
    let report_strict = vi_verify::verify_trace_at_level(&key, &trace, VerificationLevel::C, DEFAULT_ATTENTION_EPSILON);
    assert!(
    ...
    let report_tolerant = vi_verify::verify_trace_at_level_with_attn_tolerance(
...
fn test_level_a_ignores_attention_issues() {
    ...
    let report = vi_verify::verify_trace_at_level(&key, &trace, VerificationLevel::A, DEFAULT_ATTENTION_EPSILON);
