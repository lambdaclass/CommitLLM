...
fn test_honest_single_token_kv_self_consistency() {
    ...
    let attn_tol = vi_core::attention::AttentionToleranceConfig { max_abs_diff: 0 };
    let attn_failures = vi_verify::verify_attention(&key, &trace, &attn_tol);
    ...
    let kv_failures = vi_verify::verify_kv_chain(&opened);
    assert!(
...
fn test_honest_multi_token_kv_chain_passes() {
    ...
    let traces: Vec<vi_core::types::TokenTrace> = all_layers
        .iter()
        .enumerate()
        .map(|(t, layers)| {
            let final_hidden = requantize(&layers.last().unwrap().ffn_out);
    ...
    let opened: BTreeMap<u32, &vi_core::types::TokenTrace> =
...
fn test_tampered_self_kv_k_detected() {
    ...
    let attn_tol = vi_core::attention::AttentionToleranceConfig { max_abs_diff: 0 };
    let failures = vi_verify::verify_attention(&key, &trace, &attn_tol);
    assert!(
...
fn test_tampered_prior_kv_k_detected() {
    ...
    let failures = vi_verify::verify_kv_chain(&opened);
    assert!(
...
fn test_fabricated_kv_from_different_input() {
    ...
    let k_a = vi_test_vectors::requantize(&all_layers_a[0][0].k);
    let k_b = vi_test_vectors::requantize(&all_layers_b[0][0].k);
...
fn test_level_a_ignores_kv_chain() {
    ...
    let report =
        vi_verify::verify_trace_at_level(&key, &trace, VerificationLevel::A, DEFAULT_ATTENTION_EPSILON);
