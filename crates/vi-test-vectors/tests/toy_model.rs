...
fn test_merkle_proof_valid() {
    ...
    let leaf = vi_core::merkle::hash_leaf(&data);
    ...
    for t in 0..10u32 {
        if t == 3 {
            ...
            leaves.push(vi_core::merkle::hash_leaf(&t.to_le_bytes()));
        }
    }
    let tree = vi_core::merkle::build_tree(&leaves);

    assert!(vi_core::merkle::verify(
        &tree.root,
        &leaf,
        &trace.merkle_proof
    ));
...
fn test_key_serialization_roundtrip() {
    ...
    let data = vi_core::serialize::serialize_key(&key);
    let key2 = vi_core::serialize::deserialize_key(&data).unwrap();
