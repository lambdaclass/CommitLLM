...
use vi_core::sampling::{self, DecodeParams};
use vi_test_vectors::*;
use vi_verify;
...
fn test_honest_sampling_replay_passes() {
    ...
    let failures = vi_verify::verify_sampling(&key, &trace, &params, &batch_seed);
...
fn test_wrong_seed_detected() {
    ...
    if honest_tid != wrong_tid {
        let failures = vi_verify::verify_sampling(&key, &trace, &params, &wrong_seed);
...
fn test_wrong_temperature_detected() {
    ...
    if honest_tid != wrong_tid {
        let failures = vi_verify::verify_sampling(&key, &trace, &wrong_params, &batch_seed);
...
fn test_missing_lm_head_detected() {
    ...
    let params = DecodeParams { temperature: 0.0, top_k: 0, top_p: 1.0 };
    let failures = vi_verify::verify_sampling(&key, &trace, &params, &[0u8; 32]);
    assert!(!failures.is_empty());
...
fn test_replay_with_different_manifest_params() {
    ...
    // Honest passes
    let failures = vi_verify::verify_sampling(&key, &trace, &honest_params, &batch_seed);
    ...
    if alt_tid != honest_tid {
        let failures = vi_verify::verify_sampling(&key, &trace, &alt_params, &batch_seed);
