# CommitLLM: Provenance for Open-Weight LLM Inference
...
CommitLLM is a cryptographic commit-and-audit protocol for open-weight LLM inference. It is designed to keep provider overhead close to ordinary inference and verifier work CPU-feasible. The provider keeps the normal GPU serving path: no proving circuit, no per-response proof generation, and no required kernel rewrite. Instead, a provider serves a response normally, returns a compact receipt, and only opens trace data when challenged.

This targets the gap between two unsatisfying extremes:

- fingerprinting and other statistical heuristics, which can provide evidence but not exact per-response verification
- SNARK-style proof systems, which offer stronger proof objects but impose prover costs that remain too high for practical production LLM serving

Later, a verifier can challenge specific token positions and layers, request an opening, and check the opened computation against:
...
> The served trace is commitment-bound end-to-end. Within that binding, large linear components are verified by verifier-secret, information-theoretically sound algebraic checks, supported nonlinear components are checked by canonical re-execution, the attention interior is approximate, and routine prefix/KV provenance is statistical unless deep audit is used.
...
CommitLLM turns “we ran this model” from a dashboard claim into a cryptographically auditable, client-held statement.
...
with arithmetic in a finite field `F_p`; in the current implementation the prime is `p = 2^32 - 5`. If `z = W @ x`, the equality always holds. If the provider used the wrong weights or the wrong output, it fails except with probability at most `1/p`.

This is an information-theoretically sound check: if the provider uses the wrong weights or wrong output, the false-accept probability is at most `1/p`. In CommitLLM, Freivalds is the verifier-secret algebraic check inside a broader cryptographic protocol; commitments, Merkle proofs, and challenge timing provide the cryptographic binding. That matters because transformers are mostly matrix multiplication. Once those multiplications become cheap to audit, the verifier can check model identity without rerunning the full model.

In the final CommitLLM protocol, Freivalds is used for:
...
CommitLLM does not only bind “some model ran.” It binds the whole deployment surface that affects outputs.
...
CommitLLM has four guarantee classes:
...
So the honest top-line claim is not “uniformly exact end-to-end.” It is “commitment-bound end-to-end, with verifier-secret, information-theoretically sound algebraic checks for large linear components, canonical re-execution for supported nonlinear components, approximate attention replay, statistical routine prefix/KV provenance, and fail-closed handling for unsupported semantics.”
...
x_attn_i8 bridge
W_q, W_k, W_v
Dequantize + bias on Q, K, V (RoPE on Q, K)
Attention (Q@K^T, softmax, α@V)     ← the only approximate stage
committed attn_out_i8 (replay-checked)
W_o
Residual add
RMSNorm
W_gate, W_up
...
The verifier key is secret. If the provider learns the verifier’s Freivalds vectors, the linear checks can be forged.

Verification is therefore client-held rather than a public proof: the provider answers an interactive audit, and only the verifier who holds the secret key material can check it by default. The receipt alone is not a succinct SNARK-like transferable proof. However, the protocol is transferable in a weaker sense: if a full audit transcript is disclosed, third parties can independently re-check that disclosed audit against the public weights and committed receipt.

### Phase 1: Commitment

The provider runs inference normally. CommitLLM adds a tracing layer that captures the retained state needed for later audit.
...
2. **Shell verification.** Run Freivalds on the shell matmuls, verify the actual INT8 bridge tensors exactly, and canonically recompute Q/K/V dequantization, bias handling, RoPE, RMSNorm, and SiLU.
...
This is the exact core of the protocol; attention replay remains approximate, and routine prefix/KV coverage remains statistical unless deep audit is used.
...
CommitLLM still constrains it strongly:
...
On the corrected replay path, the measured attention corridor is already small on two major families:
...
These results matter because they show the core protocol architecture is already working on two important families. The remaining work is no longer to rescue a broken design, but to tighten and generalize a functioning one.
...
CommitLLM is a sidecar protocol, not a zk proof system. The serving path still runs ordinary inference. The provider pays for:
...
CommitLLM chooses a different point in the design space:
...
It is deliberately not a succinct SNARK-like proof system. Instead, transferability comes from disclosed audit evidence: a fully disclosed audit transcript can be re-checked by third parties as a bulky audit bundle.

Ongoing follow-on work includes Lean formalization, validation on additional model families and larger checkpoints, stronger KV provenance, and `W_o` conditioning.
...
CommitLLM does not assume honest provider hardware or honest provider runtime behavior. Those are what the protocol is meant to eliminate.
...
## What remains to strengthen

The main remaining work is:

- one additional family control beyond Qwen and Llama
- one materially larger model datapoint
- `W_o` conditioning analysis, so the remaining adversarial freedom after the attention corridor is explicitly bounded
- stronger KV provenance, so committed prefix state is more tightly tied to real computation
- score witnessing as a deeper audit object, if it materially tightens the current corridor

Those are important for the strongest final security claim, but they are follow-on work on top of a protocol that is already functioning on the corrected path.

The code is at [github.com/lambdaclass/CommitLLM](https://github.com/lambdaclass/CommitLLM). The implementation roadmap for reaching this final protocol is in the repository’s `roadmap.md`.
